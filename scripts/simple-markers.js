...
L.Control.SimpleMarkers = L.Control.extend({
  ...
  onAdd: function(map) {
    ...
    if (this.options.add_control) {
      var add_marker_div = L.DomUtil.create(
        'div',
        'add_marker_control',
        marker_container
      );
      ...
      L.DomEvent.addListener(
        ...
        .addListener(
          add_marker_div,
          'click',
          this.enterAddMarkerMode.bind(this)
        );
    }
    if (this.options.delete_control) {
      var del_marker_div = L.DomUtil.create(
        'div',
        'del_marker_control',
        marker_container
      );
      ...
      L.DomEvent.addListener(
        ...
        .addListener(
          del_marker_div,
          'click',
          this.enterDelMarkerMode.bind(this)
        );
    }
    ...
    if (this.markerList !== '') {
      for (var marker = 0; marker < this.markerList.length; marker++) {
        if (typeof this.markerList[marker] !== 'undefined') {
          this.markerList[marker].removeEventListener(
            'click',
  ...
  enterDelMarkerMode: function() {
    ...
    for (var marker = 0; marker < this.markerList.length; marker++) {
      if (typeof this.markerList[marker] !== 'undefined') {
        this.markerList[marker].addEventListener(
          'click',
          this.onMarkerClickDelete.bind(this)
        );
        this.map._container.style.cursor = 'crosshair';
